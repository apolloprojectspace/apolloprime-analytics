...
export default function Provider({ children }) {
  ...
  const updateEthPrice = useCallback((ftmPrice, oneDayPrice, ftmPriceChange) => {
    dispatch({
...
async function getGlobalData(ftmPrice, oldEthPrice) {
  ...
  try {
    ...
    data = result.data.wraithFactories[0]

    // console.log('Total Txs', data.totalTransactions)
    ...
    oneDayData = oneDayResult.data.wraithFactories[0]

    console.log(oneDayData.totalTransactions)

    let twoDayResult = await client.query({
      query: GLOBAL_DATA(twoDayBlock?.number),
      fetchPolicy: 'cache-first',
    })
    twoDayData = twoDayResult.data.wraithFactories[0]

    let oneWeekResult = await client.query({
      query: GLOBAL_DATA(oneWeekBlock?.number),
      fetchPolicy: 'cache-first',
    })
    const oneWeekData = oneWeekResult.data.wraithFactories[0]

    let twoWeekResult = await client.query({
      query: GLOBAL_DATA(twoWeekBlock?.number),
      fetchPolicy: 'cache-first',
    })
    const twoWeekData = twoWeekResult.data.wraithFactories[0]
    ...
    if (data && oneDayData && twoDayData) {
      ...
      // const [oneDayTxns] = oneDayData.totalTransactions
      ...
      data.txCount = oneDayData.totalTransactions
...
const getChartData = async (oldestDateToFetch, offsetData) => {
  ...
  try {
    while (!allFound) {
      ...
      skip += 1000
      data = data.concat(result.data.wraithSwapDayDatas)
      if (result.data.wraithSwapDayDatas.length < 1000) {
        allFound = true
      }
    }
...
const getEthPrice = async () => {
  ...
  try {
    ...
    priceChangeFTM = getPercentChange(currentPrice, oneDayBackPrice)
  ...
  return [ftmPrice, ftmPriceOneDay, priceChangeFTM]
}
