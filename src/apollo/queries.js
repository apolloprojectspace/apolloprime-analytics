...
export const SUBGRAPH_HEALTH = gql`
  query health {
    indexingStatusForCurrentVersion(subgraphName: "eerieeight/spookyswap") {
      synced
      health
      chains {
        chainHeadBlock {
...
export const GLOBAL_CHART = gql`
  query wraithSwapDayDatas($startTime: Int!, $skip: Int!) {
    wraithSwapDayDatas(first: 1000, skip: $skip, where: { date_gt: $startTime }, orderBy: date, orderDirection: asc) {
      id
      date
      totalVolumeUSD
      dailyVolumeUSD
...
export const GLOBAL_DATA = (block) => {
  const queryString = ` query wraithFactories {
      wraithFactories(
...
export const ALL_PAIRS = gql`
  query pairs($skip: Int!) {
    pairs(first: 150, skip: $skip, orderBy: trackedReserveFTM, orderDirection: desc) {
      id
      token0 {
        id
        symbol
...
export const PAIRS_BULK = gql`
  ...
  query pairs($allPairs: [Bytes]!) {
    pairs(first: 150, where: { id_in: $allPairs }, orderBy: trackedReserveFTM, orderDirection: desc) {
      ...PairFields
    }
  }
`
