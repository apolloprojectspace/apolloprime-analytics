...
export function getSwapLink(token0Address, token1Address = null) {
  if (!token1Address) {
    return `https://wraithswap.finance/swap?inputCurrency=${token0Address === '0x21be370d5312f44cb42ce377bc9b8a0cef1a4c83' ? 'FTM' : token0Address}`
  } else {
    return `https://wraithswap.finance/swap?inputCurrency=${
      token0Address === '0x21be370d5312f44cb42ce377bc9b8a0cef1a4c83' ? 'FTM' : token0Address
...
export function getMiningPoolLink(token0Address) {
  return `https://wraithswap.finance`
}

export function getUniswapAppLink(linkVariable) {
  let baseUniswapUrl = 'https://wraithswap.finance'
  if (!linkVariable) {
    return baseUniswapUrl
  }

  return `${baseUniswapUrl}/FTM/${linkVariable}`
}
